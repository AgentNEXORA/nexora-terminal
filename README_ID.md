...
## 📚 Dokumentasi Bahasa
- **[Dokumentasi Bahasa Indonesia](README_ID.md)** - Panduan lengkap dalam Bahasa Indonesia (Saat ini)
...
## Agen: python -m nexora_agent --prompt "..."

Instal modul agen Anda di venv khusus (direkomendasikan):
```bash
...
Backend HANYA menjalankan perintah yang terdaftar di `backend/commands.json`. Gunakan `{{placeholders}}` di dalam string perintah dan kirim argumen dari UI.

### Sinkronisasi commands.json dari GitHub resmi (opsional)
```bash
curl -fsSL "https://raw.githubusercontent.com/ORG/REPO/main/commands.json" -o /opt/nexora-console/backend/commands.json
```

## Catatan keamanan (direkomendasikan)
- Jalankan backend sebagai user non-root.
- Jaga bridge API di balik reverse proxy / jaringan internal.
- Pertimbangkan menambahkan auth API key jika diekspos di luar localhost.